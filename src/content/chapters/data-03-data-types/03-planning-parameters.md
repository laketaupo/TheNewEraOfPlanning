...
**Safety stock** — the minimum inventory buffer held to protect against uncertainty. In o9, safety stock can be set as a fixed quantity, a number of days of demand, or a statistical calculation based on variability. The parameter determines the floor below which o9 will generate a replenishment order.
...
**Reorder point** — the inventory level that triggers a new order. In some planning systems, this is calculated dynamically based on lead time and safety stock. In o9, it is typically derived from the planning engine's requirements calculation rather than set as a static parameter.
...
In the four-system landscape, planning parameters are defined in Pimcore and synced to o9. Changes should be made in Pimcore, not directly in o9, to ensure they persist through the next synchronisation cycle. If a parameter needs to be adjusted urgently (e.g., a supplier's lead time has changed immediately), the correct process is to update Pimcore and trigger a sync — not to change it directly in o9 and wait for Pimcore to be updated later.
